...
const REPO    = 'https://github.com/shahzebyc3/SHAHBAZ-MD';
const WEBSITE = '';
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        const botName  = getStr('botName') || config.BOT_NAME || 'SHAHBAZ MD';
        const ownerNum = `+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}`;
        const mode     = (config.MODE || 'public').toUpperCase();
        const pfx      = prefix || '.';
        const imgUrl   = getStr('pic1') || config.ALIVE_IMG || 'https://files.catbox.moe/l7d81a.png';
