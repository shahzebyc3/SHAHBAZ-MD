...
module.exports = {
    ...
    run: async (sock, message, args, { sender, contextInfo }) => {
        ...
        const theme  = (config.THEME || 'SHAHBAZ-MD').toLowerCase();

        const now    = moment().tz(TZ);
        const date   = now.format('ddd, D MMM YYYY');
        const time   = now.format('hh:mm A');

        const botName  = t.botName  || 'SHAHBAZ MD';
        const footer   = t.footer   || 'SHAHBAZ MD';
        const alive    = t.alive    || 'System online. All functions nominal.';
        const emoji    = t.aliveEmoji || '⚡';
        const imageUrl = config.ALIVE_IMG || t.pic1 || 'https://files.catbox.moe/l7d81a.png';
