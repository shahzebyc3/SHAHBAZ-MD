...
const globalContextInfo = {
    ...
    forwardedNewsletterMessageInfo: {
        ...
        newsletterName: '◢◤ SHAHBAZ Tech Nexus ◢◤',
        serverMessageId: 144
    }
};
...
function generateFancyBio() {
    ...
    const bios = [
        ...
        `⚡ SHAHBAZ MD Active ✦ ${timeStr} ✦ ${dateStr} ✦`,
        `💫 ${config.BOT_NAME} Operational ✦ ${dateStr} ✦`,
        `🚀 SHABAZ MD Live ✦ ${dateStr} ✦ ${timeStr} ✦`,
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('connection.update', async update => {
        ...
        if (connection === 'close') {
            ...
            setTimeout(async () => {
                const nlJid = '120363424231270188@newsletter';
            ...
            for (const m of messages) {
                ...
                if (isNewsletter) {
                    if (process.uptime() > 25) {
                        ...
                        if (serverId) {
                            const isOwnNewsletter = nlJid === '120363424231270188@newsletter';
...
(async () => {
    try {
        ...
        console.log('║     githubSHAHBAZ-MD V2           ║');
        console.log('╚══════════════════════════════════════════╝');
        console.log('\x1b[0m');
        logMessage('INFO', 'Booting SHAHBAZ MD Bot...');
